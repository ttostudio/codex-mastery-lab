...
const appUrl = process.env.AIDD_MVP014_APP_URL ?? "http://127.0.0.1:3014";
...
for (const scenario of ["empty", "valid", "failure", "timeout", "rate_limit"]) {
  ...
  await shot(`aidd-control-plane-mvp014-${scenario}.png`);
}
...
await page.setContent(`<!doctype html><meta charset="utf-8"><style>body{margin:0;background:#0f172a;color:#e2e8f0;font:22px/1.55 ui-monospace,Menlo,monospace}.card{padding:36px}h1{font:800 32px system-ui;margin:0 0 20px;color:#bfdbfe}.ok{color:#bbf7d0}pre{white-space:pre-wrap;overflow-wrap:anywhere}</style><div class="card"><h1>AIDD Control Plane MVP 014 terminal evidence</h1><p class="ok">独立Mock CI Service契約の検証ログ。</p><pre>${escaped}</pre></div>`);
await shot("aidd-control-plane-mvp014-terminal-evidence.png");
await browser.close();
