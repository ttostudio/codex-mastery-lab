...
test("初期empty stateが表示される", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "独立Mock CI Serviceで、CI証跡の取得成功・不足・制限状態を判断するSaaS" })).toBeVisible();
...
test("Mock CI Evidence Connectorでvalid failure timeoutを切り替えられる", async ({ page }) => {
  ...
  await expect(page.getByText("14014").first()).toBeVisible();
  ...
  await expect(page.getByText("手動Evidence Binderへterminal evidence")).toBeVisible();
});
