  ...
  return (
    <main>
      <section className="hero" aria-labelledby="hero-title">
        <div>
          <p className="eyebrow">AIDD Control Plane MVP 014</p>
          <h1 id="hero-title">独立Mock CI Serviceで、CI証跡の取得成功・不足・制限状態を判断するSaaS</h1>
          <p>
            Project Intake Wizardは、初めてAIDD Control Planeを使う人が「何を入力すればよいか」を順番に答えられる画面です。
            App Type Templatesで初期値を作り、GitHub Actions Artifact Fetch Planを壊さず、coverage、playwright-report、test-results、terminal-evidence、empty/valid/failure/timeout/rate_limit screenshotの不足をAIDD-Spec v0.1の修理計画として束ねます。
          </p>
        </div>
      ...
      <section className={`artifact-binder artifact-${mockCiMode === "valid" ? "valid" : mockCiMode === "empty" ? "empty" : "failure"}`} aria-labelledby="mock-ci-title">
        <div className="section-heading">
          ...
          <p>NEXT_PUBLIC_MOCK_CI_SERVICE_URLまたはhttp://127.0.0.1:4314から状態を取得し、empty / valid / failure / timeout / rate_limitをローカルUIへ反映します。</p>
        </div>
        <div className={`verification-summary ${mockCiConnection === "service" ? "is-ready" : "is-not-ready"}`} aria-live="polite">
          ...
          <span>{mockCiConnection === "fallback" ? "取得失敗時は手動Artifact Evidence Binderでterminal evidenceとscreenshot evidenceを添付します。" : `接続先: ${mockCiServiceUrl}`}</span>
        </div>
