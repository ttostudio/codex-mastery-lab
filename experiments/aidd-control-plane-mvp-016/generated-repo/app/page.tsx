  ...
  return (
    <main>
      <section className="hero" aria-labelledby="hero-title">
        <div>
          <p className="eyebrow">AIDD Control Plane MVP 016</p>
          <h1 id="hero-title">CI workflowとartifact保存を静的監査し、証跡不足を次回依頼へ戻すSaaS</h1>
          <p>
            Project Intake Wizardは、初めてAIDD Control Planeを使う人が「何を入力すればよいか」を順番に答えられる画面です。
            App Type Templatesで初期値を作り、GitHub Actions Artifact Fetch PlanとCI Workflow Artifact Auditorを壊さず、coverage、playwright-report、test-results、experiments terminal evidence、empty/valid/failure screenshotの不足をAIDD-Spec v0.1の修理計画として束ねます。
          </p>
        </div>
      ...
      <section className={`artifact-binder artifact-${workflowAudit.status}`} aria-labelledby="workflow-auditor-title">
        ...
        <div className={`verification-summary ${workflowAudit.status === "valid" ? "is-ready" : "is-not-ready"}`} aria-live="polite">
          ...
          <span>{workflowAuditor.workflowPath || ".github/workflows/aidd-control-plane.yml未接続"} / capture:mvp016: {workflowAudit.missingCaptureCommand ? "未接続" : "接続済み"}</span>
        </div>
