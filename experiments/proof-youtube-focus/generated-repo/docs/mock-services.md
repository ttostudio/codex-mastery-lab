...
`playwright.config.ts` の `webServer.command` は `pnpm run mock:start` を先に実行し、`STUDYSTREAM_USE_EXTERNAL_MOCKS=1` を付けてNext.js dev serverを起動します。`/states` は独立mock serviceの認証、課金、ネットワーク状態を読み、`/watch/:id` はmock-mediaの状態を読みます。
...
- `STUDYSTREAM_MOCK_NODE_ONLY=1` でNode fallbackを起動し、4 serviceの `/health` を確認する。

## ライブラリ永続化の将来境界

StudyStreamの学習履歴、保存、集中キューはUI検証を優先し、localStorageに保存します。将来mock APIへ移す場合は、動画データAPIとは分けて `mock-library` を追加し、ユーザー単位の履歴、保存、プレイリスト項目を扱う契約にします。
