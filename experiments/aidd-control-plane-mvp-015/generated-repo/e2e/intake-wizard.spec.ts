...
test("MVP 015の初期empty stateとfixture駆動contractが表示される", async ({ page }) => {
  await page.goto("/");

  await expect(page.getByRole("heading", { name: "fixture駆動Mock CI Serviceで、CI証跡の取得成功・不足・制限状態を判断するSaaS" })).toBeVisible();
