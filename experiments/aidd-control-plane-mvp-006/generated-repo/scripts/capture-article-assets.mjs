...
function renderTerminalEvidence() {
  const logs = [
    ...
    const filePath = join(experiment, 'artifacts', 'terminal', file);
    const text = (existsSync(filePath) ? readFileSync(filePath, 'utf8') : `${label}: ログ未生成`)
...
</style></head><body><div class="card"><h1>AIDD Control Plane MVP 006 terminal evidence <span class="ok">PASS</span></h1><pre>${logs.replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</pre></div></body></html>`;
}
...
try {
  ...
  await page.screenshot({ path: join(screenshotDir, 'aidd-control-plane-mvp006-empty.png'), fullPage: true });
  ...
  await page.screenshot({ path: join(screenshotDir, 'aidd-control-plane-mvp006-ready.png'), fullPage: true });

  await page.getByRole('button', { name: '失敗サンプルを適用' }).click();
  await page.screenshot({ path: join(screenshotDir, 'aidd-control-plane-mvp006-failure.png'), fullPage: true });

  await page.getByRole('button', { name: '証跡不足サンプルを適用' }).click();
  await page.screenshot({ path: join(screenshotDir, 'aidd-control-plane-mvp006-evidence-missing.png'), fullPage: true });
  ...
  await term.screenshot({ path: join(screenshotDir, 'aidd-control-plane-mvp006-terminal-evidence.png'), fullPage: true });
  await browser.close();
